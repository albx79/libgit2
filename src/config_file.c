...
static refcounted_strmap *refcounted_strmap_alloc(void)
{
	refcounted_strmap *map;
...
{
	refcounted_strmap *values, *tmp;
	diskfile_backend *b = (diskfile_backend *)cfg;
	struct reader *reader = NULL;
	int error = 0;

	values = refcounted_strmap_alloc();
	GITERR_CHECK_ALLOC(values);
